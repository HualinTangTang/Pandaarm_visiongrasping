...
from cs import CanonicalSystemcd, CanonicalSystem
...
if __name__ == "__main__":
    ...
    # read document from position trajectory txt

    data = np.genfromtxt("/home/mirmi/teachingdata/demo_position.txt", dtype=[float, float, float])  # 将文件中数据加载到data数组里
    print(data)
    data_list = list(data)
    print(data_list)
    x_position = []
    ...
    z_position = []
    for i in data_list:
        x_position.append(i[0])
    print(x_position)
    for j in data_list:
        y_position.append(j[1])
    print(y_position)
    for k in data_list:
        z_position.append(k[2])
    print(z_position)
    data_len = len(x_position)
  ...
  # ----------------- For different initial and goal positions
    ...
    y_reproduce_2, dy_reproduce_2, ddy_reproduce_2 = dmp.reproduce(tau=0.5, initial=[0.306994, 0.0011577, 0.487114 ], goal=[0.279167, 0.526884, 0.143794 ])
    plt.figure(figsize=(10, 5))
    plt.plot(y_demo[0,:], 'g', label='demo x_position')
    plt.plot(y_reproduce[:,0], 'r--', label='reproduce x_position')
    plt.plot(y_reproduce_2[:,0], 'r-.', label='reproduce 2 x_position')
    plt.plot(y_demo[1,:], 'b', label='demo y_position')
    plt.plot(y_reproduce[:,1], 'm--', label='reproduce y_position')
    plt.plot(y_reproduce_2[:,1], 'm-.', label='reproduce 2 y_position')

    plt.plot(y_demo[2,:], 'c', label='demo z_position')
    plt.plot(y_reproduce[:,2], 'y--', label='reproduce z_position')
    plt.plot(y_reproduce_2[:,2], 'y-.', label='reproduce 2 z_position')
    plt.legend()
    plt.grid()
    plt.xlabel('timestep')
    plt.ylabel('y')
    ...
    #save reproduce data into txt file
    for i in tqdm(y_demo[0,:]):
        f = open('/home/mirmi/dmp_learndata/demox.txt', 'a')
        f.write(str(i) + '\n')
        f.close()

    for i in tqdm(y_reproduce[:,0]):
        f = open('/home/mirmi/dmp_learndata/reprox.txt', 'a')
        f.write(str(i) + '\n')
        f.close()

    for i in tqdm(y_reproduce_2[:,0]):
        f = open('/home/mirmi/dmp_learndata/repro2x.txt', 'a')
        f.write(str(i) + '\n')
        f.close()

    for i in tqdm(y_demo[1,:]):
        f = open('/home/mirmi/dmp_learndata/demoy.txt', 'a')
        f.write(str(i) + '\n')
        f.close()

    for i in tqdm(y_reproduce[:,1]):
        f = open('/home/mirmi/dmp_learndata/reproy.txt', 'a')
        f.write(str(i) + '\n')
        f.close()

    for i in tqdm(y_reproduce_2[:,1]):
        f = open('/home/mirmi/dmp_learndata/repro2y.txt', 'a')
        f.write(str(i) + '\n')
        f.close()

    for i in tqdm(y_demo[2, :]):
        f = open('/home/mirmi/dmp_learndata/demoz.txt', 'a')
        f.write(str(i) + '\n')
        f.close()

    for i in tqdm(y_reproduce[:, 2]):
        f = open('/home/mirmi/dmp_learndata/reproz.txt', 'a')
        f.write(str(i) + '\n')
        f.close()

    for i in tqdm(y_reproduce_2[:, 2]):
        f = open('/home/mirmi/dmp_learndata/repro2z.txt', 'a')
        f.write(str(i) + '\n')
        f.close()
    ...
    # set new initial and goal poisitions
    y_reproduce_2, dy_reproduce_2, ddy_reproduce_2 = dmp.reproduce(tau=0.8, initial=[0.306816, 0.00127088, 0.486804 ], goal=[0.268586, 0.621814, 0.147611 ])

    plt.figure(figsize=(10, 5))
    plt.plot(y_demo[0,:], 'g', label='demo x_position')
    plt.plot(y_reproduce[:,0], 'r--', label='same reproduce x_position')
    plt.plot(y_reproduce_2[:,0], 'r-.', label='same reproduce 2 x_position')
    plt.plot(y_demo[1,:], 'b', label='demo y_position')
    plt.plot(y_reproduce[:,1], 'm--', label='same reproduce y_position')
    plt.plot(y_reproduce_2[:,1], 'm-.', label='same reproduce 2 y_position')
    plt.plot(y_demo[2, :], 'c', label='demo y_position')
    plt.plot(y_reproduce[:, 2], 'y--', label='same reproduce z_position')
    plt.plot(y_reproduce_2[:, 2], 'y-.', label='same reproduce 2 z_position')
